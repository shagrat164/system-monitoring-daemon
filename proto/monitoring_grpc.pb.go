...
const (
	Monitoring_GetStats_FullMethodName = "/proto.Monitoring/GetStats"
)
...
func (c *monitoringClient) GetStats(ctx context.Context, in *StatsRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[StatsResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Monitoring_ServiceDesc.Streams[0], Monitoring_GetStats_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Monitoring_GetStatsClient = grpc.ServerStreamingClient[StatsResponse]
...
func (UnimplementedMonitoringServer) GetStats(*StatsRequest, grpc.ServerStreamingServer[StatsResponse]) error {
...
func _Monitoring_GetStats_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StatsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(MonitoringServer).GetStats(m, &grpc.GenericServerStream[StatsRequest, StatsResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Monitoring_GetStatsServer = grpc.ServerStreamingServer[StatsResponse]
